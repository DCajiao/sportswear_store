...
CREATE TABLE IF NOT EXISTS `Sportswear_Store`.`Compra` (
  ...
  `Cuenta_Usuario_Identificacion` INT NOT NULL,
  PRIMARY KEY (`ID_compra`, `Cuenta_Usuario`, `Cuenta_Usuario_Identificacion`),
  INDEX `fk_Compra_Cuenta1_idx` (`Cuenta_Usuario` ASC, `Cuenta_Usuario_Identificacion` ASC) VISIBLE,
  CONSTRAINT `fk_Compra_Cuenta1`
    FOREIGN KEY (`Cuenta_Usuario` , `Cuenta_Usuario_Identificacion`)
    REFERENCES `Sportswear_Store`.`Cuenta` (`Usuario` , `Usuario_Identificacion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Sportswear_Store`.`Almacena` (
  `Producto_Identificacion` INT NULL,
